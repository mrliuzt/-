...
void RCC522_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;//配置RST
...
//功能描述向RC522的某一寄存器写一个字节数据
//输入参数addr--寄存器地址val--要写入的值
void Write_RC522(u8 addr, u8 val) 
{
	//地址格式0XXXXXX0  
	RC522_CS(0);   
	MySPI_SwapByte((addr<<1)&0x7E);
	MySPI_SwapByte(val);   
	RC522_CS(1); 
}

//功能描述从RC522的某一寄存器读一个字节数据
//输入参数addr--寄存器地址
//返 回 值返回读取到的一个字节数据 
u8 Read_RC522(u8 addr) 
{  
	u8 val;
	//地址格式1XXXXXX0   
	RC522_CS(0);     
	MySPI_SwapByte(((addr<<1)&0x7E)|0x80);   
	val=MySPI_SwapByte(0xFF);    
	RC522_CS(1); 
	return val;
}
...
//初始化
void RC522_Initializtion(void) 
{
	MySPI_Init(); 
	RC522_Reset();         
	//Timer: TPrescaler*TreloadVal/6.78MHz = 0xD3E*0x32/6.78=25ms     
	Write_RC522(TModeReg,0x8D);				//TAuto=1为自动计数模式，受通信协议影向。低4位为预分频值的高4位
	//Write_RC522(TModeReg,0x1D);				//TAutoRestart=1为自动重载计时，0x0D3E是0.5ms的定时初值//test    
	Write_RC522(TPrescalerReg,0x3E); 	//预分频值的低8位     
	Write_RC522(TReloadRegL,0x32);		//计数器的低8位                
	Write_RC522(TReloadRegH,0x00);		//计数器的高8位       
	Write_RC522(TxAutoReg,0x40); 			//100%ASK     
	Write_RC522(ModeReg,0x3D); 				//CRC初始值0x6363
	Write_RC522(CommandReg,0x00);			//启动RC522  
	//Write_RC522(RFCfgReg, 0x7F);    //RxGain = 48dB调节卡感应距离      
	AntennaOn();          							//打开天线 
}
...
//					BackLen--返回数据的位长度
//返 回 值成功返回MI_O
u8 RC522_ToCard(u8 command, u8 *sendData, u8 sendLen, u8 *backData, u16 *backLen) 
{
	u8  status=MI_ERR;
	u8  irqEn=0x00;
	u8  waitIRq=0x00;
	...
	//
	Write_RC522(ComIEnReg, irqEn|0x80);		//允许中断请求     
	ClearBitMask(ComIrqReg, 0x80);  				//清除所有中断请求位               	
	SetBitMask(FIFOLevelReg, 0x80);  				//FlushBuffer=1, FIFO初始化
	Write_RC522(CommandReg, PCD_IDLE); 		//使RC522空闲   
	//向FIFO中写入数据     
	for (i=0; i<sendLen; i++)
	...
	//天线发送数据     
	if (command == PCD_TRANSCEIVE)					//如果是卡片通信命令，RC522开始向天线发送数据      
	...
	if (i != 0)     
	{            
		if(!(Read_RC522(ErrorReg) & 0x1B)) //BufferOvfl Collerr CRCErr ProtecolErr         
		{            
			if (n & irqEn & 0x01)			//                  
				status = MI_NOTAGERR;		//
			//
			...
			{                 
				n = Read_RC522(FIFOLevelReg);		//n=0x02                
				lastBits = Read_RC522(ControlReg) & 0x07;	//lastBits=0               
				if (lastBits!=0)                         
	...
	//
	Write_RC522(ControlReg,0x80);				//timer stops     
...
{  
	...
	Write_RC522(BitFramingReg, 0x07);  //TxLastBists = BitFramingReg[2..0]   
	TagType[0] = reqMode;  
...
{     
	...
	Write_RC522(BitFramingReg, 0x00);  	//TxLastBists = BitFramingReg[2..0]
...
{     
	...
	pOutData[1] = Read_RC522(CRCResultRegH);
	Write_RC522(CommandReg, PCD_IDLE);
}
//功能描述选卡读取卡存储器容量
//输入参数serNum--传入卡序列号
...
//					serNum--卡片序列号4字节
//返 回 值成功返回MI_OK
u8 RC522_Auth(u8 authMode, u8 BlockAddr, u8 *Sectorkey, u8 *serNum) 
{     
	u8  status;     
	u16 recvBits;     
	u8  i;  
...
//返 回 值成功返回MI_OK
u8 RC522_Read(u8 blockAddr, u8 *recvData) 
{     
	u8  status;     
	u16 unLen;
	//      
...
//返 回 值成功返回MI_OK
u8 RC522_Write(u8 blockAddr, u8 *writeData) 
{     
	u8  status;     
	u16 recvBits;     
	u8  i;  
