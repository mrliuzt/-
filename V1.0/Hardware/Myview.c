...
uint8_t wire_as = 0xFF;//????????λ
uint8_t wire_ic = 0xFF;//???????λ
uint8_t root = 0xFF,ASGO = 0xFF;//???????????λ
uint8_t amend = 0xFF,again = 0xFF;//???????????λ
uint8_t hid = 0xFF;//??????λ
uint8_t ok = 0xFF;//???????λ
uint8_t sos = 0xFF; // ???????λ
...
{
   if(TIM_GetITStatus(TIM2,TIM_IT_Update)==SET)// ??????TIM2??????ж?
   {
	    time_count--;
	    if(time_count==0)
		{
