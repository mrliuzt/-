...
uint8_t wire_as = 0xFF;//录入指纹标志位
uint8_t wire_ic = 0xFF;//录入卡号标志位
uint8_t root = 0xFF,ASGO = 0xFF;//管理员进入标志位
uint8_t amend = 0xFF,again = 0xFF;//修改密码功能标志位
uint8_t hid = 0xFF;//隐藏标志位
uint8_t ok = 0xFF;//解锁标志位
uint8_t sos = 0xFF; // 报警标志位

uint8_t fail_count = 0;//自锁标志位
uint8_t per = 0;//位

extern uint32_t password;
extern uint8_t matrix_num;
extern uint8_t row;//行
extern uint8_t card_numberbuf0[5];
uint32_t password_key;//初始密码

uint8_t time_count = 15;//锁屏计数
...
void View_Init(void)
{
	...
	{
		password_key = Store_Data[1]+ Store_Data[2]*65535;//从内部flash获取密码，实现掉电不丢失
		Add_Init();//从内部flash获取卡号，实现掉电不丢失
		Bee_Init();
		kaisuo_OFF();//锁头关闭，防止误开
		TIM_Cmd(TIM2,DISABLE);
		time_count = 15;
		password_clear();
		per = 0;
		HC_XR();//蓝牙牵手
		Matrix_row();//开启矩阵键盘行扫描
		MyRTC_ReadTime();
	...
	//用户模式-------------------------
		if(root == 0xFF)
		{
			...
			if(fail_count==10)
			{
			 ...
			 else if(fail_count==7)//错误次数6次
			{
				fail_count = 3;//设置几秒后解锁
				while(fail_count--)
				{
					time_count = 15;
					Delay_ms(1000);
					OLED_Clear();
					OLED_ShowChinese(15, 5,"还剩下两次机会");
			...
			else if(fail_count ==2)//错误次数2次
			{
				fail_count = 2;//设置几秒后解锁
				while(fail_count--)
				{
					time_count = 15;
					Delay_ms(1000);
					OLED_Clear();
					OLED_ShowChinese(15, 5,"还剩下六次机会");
					OLED_ShowNum(20,25,fail_count,2,OLED_8X16);
					OLED_ShowChinese(45,25,"秒后重试");
					OLED_Update();
				}
		...
		if(root == 1)
		{
			...
			while(ASGO == 0xFF)
			{
				...
				OLED_Update();//更新显示	
					...
					{
						fail_count = 30;//设置几秒后解锁
						while(fail_count--)
						{
							time_count = 15;
							Delay_ms(1000);
							OLED_Clear();
							OLED_ShowChinese(15, 5,"还剩下一次机会");
							OLED_ShowNum(20,25,fail_count,2,OLED_8X16);
							OLED_ShowChinese(45,25,"秒后重试");
							OLED_Update();
						}
					...
					{
						fail_count = 10;//设置几秒后解锁
						while(fail_count--)
						{
							time_count = 15;
							Delay_ms(1000);
							OLED_Clear();
							OLED_ShowChinese(15, 5,"还剩下三次机会");
							OLED_ShowNum(20,25,fail_count,2,OLED_8X16);
							OLED_ShowChinese(45,25,"秒后重试");
							OLED_Update();
						}
			...
			if(amend == 1)
			{
				OLED_Clear();
					...
					while(1)
					{
						...
						OLED_ClearArea(28+per*8, 23, 8*(9-per), 16);
						OLED_ClearArea(30, 47, 120, 16);
						OLED_ShowChinese(20, 47,"确认");
						OLED_ShowChinese(72, 47,"退出");
						OLED_Update();
						if(matrix_num == 50)
						{
							OLED_ShowChinese(20, 47,"确认");
							...
							OLED_Update();
							Delay_ms(250);
							password_key = password;//设置密码
							OLED_ClearArea(20, 47, 120, 16);
							OLED_ShowChinese(33, 47,"设置成功");
							matrix_num = 0xFF;
							amend = 0xFF;
							password = 0;
							per = 0;
							...
							while(1)
							{
								...
								if(matrix_num == 50)
								{
									...
									{
										OLED_ClearArea(30, 47, 120, 16);
										OLED_ShowChinese(20, 47,"密码不一致");
										OLED_Update();
										Delay_s(1);
										OLED_ShowChinese(20, 47,"请重头输入");
										OLED_Update();
										Delay_s(1);
...
void EXTI0_IRQHandler(void)//第四列
{
...
void EXTI1_IRQHandler (void)//第三列
{
